...
//定義interface，輸入為theta，輸出為sin、cos值
void cordic(
...
#pragma HLS DATAFLOW //使用dataflow的pragma來實作pipeline
#pragma HLS ALLOCATION instances = one_stage limit = 15 function
	...
	Stage0_COS.write(0.60735), Stage0_Sin.write(0.0); //寫入初值

	//需要把每個iteration都實作成是它特定的hardware，所以這個function要複製14份，而最後的output是在第15個。
