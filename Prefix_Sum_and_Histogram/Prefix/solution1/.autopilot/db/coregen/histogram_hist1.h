...
#ifndef _histogram_hist1_H_
#define _histogram_hist1_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "histogram_hist1_memcore.h"

SC_MODULE(histogram_hist1) {
...
protected:
    ...
    // buffer instances
    histogram_hist1_memcore* buffer[BufferCount];
...
public:
    ~histogram_hist1();
    SC_CTOR(histogram_hist1) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "histogram_hist1_memcore_U_";
            name += szTmp;

            buffer[i] = new histogram_hist1_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "histogram_hist1.trace");
            // ports
